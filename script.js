...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const displayProducts = (products) => {
        productGrid.innerHTML = products.map(product => `
            <div class="bg-white p-6 rounded shadow-xl flex flex-col border-4 border-indigo-500/1000">
                <img src="${product.image}" alt="${product.title}" class="h-54 w-full object-cover rounded mb-4 product-image">
                <h2 class="text-lg font-bold">${product.title}</h2>
                <p class="text-gray-600">${product.vendor}</p>
                <div class="flex justify-between items-center mt-4">
                    <span class="text-green-600 font-bold">₹${product.price}</span>
                    ${product.compare_at_price ? `<span class="text-red-600 line-through">₹${product.compare_at_price}</span>` : ''}
                </div>
                ${product.badge_text ? `<span class="bg-yellow-200 text-yellow-800 text-sm px-2 py-1 rounded">${product.badge_text}</span>` : ''}
                <div class="flex-grow"></div>
                <button class="add-to-cart-btn mt-4 py-2 px-4 rounded w-full" data-id="${product.id}">Add to Cart</button>
            </div>
        `).join('');
    };

    const allProducts = apiData.categories.flatMap(category => category.category_products);
    ...
    const filterAndDisplayProducts = () => {
        ...
        if (selectedCategory !== 'All') {
            const categoryProducts = apiData.categories.find(cat => cat.category_name === selectedCategory).category_products;
            filteredProducts = filteredProducts.filter(product => categoryProducts.includes(product));
        }
    ...
    const saveCartItems = (cartItems) => localStorage.setItem('cartItems', JSON.stringify(cartItems));
    ...
    const displayCartItems = () => {
        ...
        cartItemsContainer.innerHTML = cartItems.map(item => `
            <div class="bg-white p-6 rounded shadow-xl flex flex-col border-4 border-indigo-500/1000">
                <img src="${item.image}" alt="${item.title}" class="h-54 w-full object-cover rounded mb-4">
                <h2 class="text-lg font-bold">${item.title}</h2>
                <p class="text-gray-600">${item.vendor}</p>
                <div class="flex justify-between items-center mt-4">
                    <span class="text-green-600 font-bold">₹${item.price}</span>
                    ${item.compare_at_price ? `<span class="text-red-600 line-through">₹${item.compare_at_price}</span>` : ''}
                </div>
                <div class="flex items-center mt-4">
                    <button class="decrease-quantity-btn py-1 px-3 bg-red-500 text-white rounded">-</button>
                    <span class="mx-2">${item.quantity}</span>
                    <button class="increase-quantity-btn py-1 px-3 bg-green-500 text-white rounded">+</button>
                </div>
                <button class="remove-from-cart-btn mt-4 py-2 px-4 bg-red-500 text-white rounded">Remove</button>
            </div>
        `).join('');
    };
    ...
    cartIcon.addEventListener('click', () => {
        productGrid.classList.toggle('hidden');
    ...
    cartItemsContainer.addEventListener('click', (e) => {
        const cartItems = getCartItems();
        const itemIndex = Array.from(cartItemsContainer.children).indexOf(e.target.closest('.bg-white'));
